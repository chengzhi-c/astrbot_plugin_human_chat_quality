...
MAX_OPEN_LEN = 20
DAY_SECONDS = 86400
# 同一 opener 在最近窗口内至少出现这么多次才视为重复信号（v0.6.0：2→3，降低误报）
...
# 结构级 AI 腔信号：仅保留强信号、低误报的连发模式（v0.6.0：移除普通句式正则）
...
def detect_cliches(text: str, custom_cliches: tuple[str, ...] = ()) -> list[str]:
    ...
    内置末尾模板仅在回复结尾命中；custom_cliches 是管理员显式词库，
    任意位置一次精确命中即提示。
    """
    ...
    for label, pattern in _PATTERN_CHECKS:
        if label in hits:
            continue
        if len(pattern.findall(normalized)) >= CONSECUTIVE_THRESHOLD:
            hits.append(label)
    return hits
