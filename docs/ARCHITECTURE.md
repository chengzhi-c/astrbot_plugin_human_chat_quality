...
过滤字符集（`_RUNTIME_ITEM_FORBIDDEN`）与判定条件同源维护：`、` 会让校验按错误边界切分，`\r`/`\n` 会被换行归一化折成换行而触发换行拒判。`tests/test_quality_rules.py` 以"任何会破坏回转资格的字符都必须被过滤，反之不得过度过滤"为不变量。

**孤儿块不影响注入**：无法核验的块（`ambiguous`）语义是"保留不清理"，用于避免误删用户手写的形似文本。该类块只影响清理统计，不清空、不阻断注入路径。若同时把它当作"已有提醒"而拒绝注入，该会话的动态提醒将长期静默且无法自愈。
