...
1. **固定规则**：写给模型的通用约束，幂等写入 system prompt。只做减法，不碰人设的性格、称呼、情绪和口头禅。采用 natural-talk v2.1.0+（MIT）「作为 System Prompt」原文（含上游后续新增的"不适用范围"行）+ 插件附加安全条款（来源与许可见 `THIRD_PARTY_NOTICES.md`）。
